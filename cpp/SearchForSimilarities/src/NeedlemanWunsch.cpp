...
NeedlemanWunsch::NeedlemanWunsch(const vector<unsigned int>& S1, const vector<unsigned int>& S2, const vector<vector<float>>& pwm, float d, float e) {
    ...
    ///////////////////
    cout << "d: " << this->d << " e: " << this->e << endl;
    ///////////////////
    

    this->L = S1.size();
    //this->nodes = vector<vector<Node*>>(this->L + 1, vector<Node*>(this->L + 1, nullptr));
    this->nodes = vector<vector<Node*>>(this->L + 1, vector<Node*>(this->S2.size() + 1, nullptr));
    ...
    for (i = 1; i <= this->L; i++) {
        //for (j = 1; j <= this->L; j++) {
        for (j = 1; j <= this->S2.size(); j++) {
            nodes[i][j] = new Node();
        }
    }

    for (i = 1; i <= this->L; i++) { nodes[i][0] = new Node((-1) * this->d * i, nodes[i - 1][0]); }
    for (j = 1; j <= this->S2.size(); j++) { nodes[0][j] = new Node((-1) * this->d * j, nodes[0][j - 1]); }
...
/**************** Decomposition ****************/
vector<tuple<unsigned int, unsigned int>>  NeedlemanWunsch::get_matrix_path() {
    vector<tuple<unsigned int, unsigned int>> matrix_path;
    unsigned int i = 100;
    unsigned int j = this->S2.size();
    Node* node;
    Node* prev_node;

    while (i >= 0 || j >= 0) {
        cout << "i: " << i << " j: " << j << endl;
        matrix_path.push_back(tuple(i, j));
        node = this->nodes[i][j];
        ...
        if (node->has_prev()) {
            ...
            else if (prev_node == this->nodes[i - 1][j - 1]) { i--; j--; }
            else if (prev_node == this->nodes[i][j - 1]) { j--; }
            else if (prev_node == this->nodes[i - 1][j]) { i--; }
        }
        else
            break;
    }
...
float NeedlemanWunsch::calc_F(unsigned int i, unsigned int j) {
    ...
    if ((i == 1 && j == 1) || i == 1) {
        ...
        if (i == 1 && j == 1) {
            ...
            r3 = this->calc_Fy(i, j - 1) + this->pwm[n1 - 1][S2[j - 1] - 1];
        }
    }    
    else {
        unsigned int ii = i - 1;
        ...
        else        r3 = this->calc_Fy(i, j - 1) + this->pwm[S1[i - 1] - 1][0];
        ...
        r2 = this->calc_Fx(i - 1, j) + this->pwm[n2 - 1][S2[j - 1] - 1];
    }

    max_r = max({ r1, r2, r3 });

    if (max_r == r1)      { this->nodes[i][j]->set_value_parent(max_r, nodes[i - 1][j - 1]); }
    else if (max_r == r2) { this->nodes[i][j]->set_value_parent(max_r, nodes[i - 1][j]); }
    else                  { this->nodes[i][j]->set_value_parent(max_r, nodes[i][j - 1]); }
